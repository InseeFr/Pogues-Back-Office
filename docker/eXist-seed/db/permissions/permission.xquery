xquery version "3.0";
(:~
 : module qui permet d'affecter des droits 'rwxrwxr-x' au r�pertoire/� la collection, pour l'utilisateur et le groupe souhait�s ou ceux par d�faut que l'ont trouve dans le fichier "/db/restxq/properties.xml" pour la base de stromae. 
 :)
...
function permission:helloworld($racine-enquete as xs:string*) as item()+{
    let $log := util:log("INFO", "permission:helloworld - racine : "||$racine-enquete||"*****************************************************")     
...
(::::::::::::::::::::::::DROITS:::::::::::::::::::::::::::::::)
(:~
 : on cr�e les users et le group, on ajoute/modifie le group comme primary group aux user,
 :
 : on set les drtoi par d�faut des en lecture + �criture pour le groupe (juste lecture au reste du monde). 
 :
 : on enl�ve les mots de passe du fichier propri�t�
 :
 : TODO Impossible de mettre les droit en ex�cution par d�faut. Pourquoi ? => quand on livre des restxq, faire appel aux modofs de permissions d'ici, sinon c'est bon
 :) 
...
(: on chnage les permission sur la liste de fichier pass� en param :)
...
declare function permission:change-permission-element($element as xs:string,$userDefault as xs:string,$groupDefault as xs:string) {
    let $modifiable := if(starts-with($element, "/db/system") or starts-with($element, "/db/apps"))then(
            ...
            (: si on veut des permisson plus sp�cifique
                let $temp := if ((contains($element, '.xqm')) or (contains($element, '.xquery'))) 
                 then sm:chmod(xs:anyURI($element), 'rwxrwxr-x') (\:on est oblig� de mettre les x pour lister les collections:\)
