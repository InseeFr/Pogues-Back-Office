...
(:~
 ...
 : @autor Marie : modif : rappatriement des functx ici, ajout helloworld
 :)
...
declare variable $unzip:version := "20171206-OR" ;

(:~
 : HELLOWORLD
 :
 : GET /unzip/helloworld
 :
 : @return 200 + <resultat><xqm>{'/unzip/helloworld'}</xqm><version>{$unzip:version}</version><message>{"Chuck Norris peut claquer des doigts de pied."}</message></resultat>
:)

declare
%rest:GET
%rest:path("/unzip/helloworld")
function unzip:helloworld() as item()+{
    let $mess:=<resultat><xqm>{'/unzip/helloworld'}</xqm><version>{$unzip:version}</version><message>{"Chuck Norris peut claquer des doigts de pied."}</message></resultat>
    return ($mess)
};
...
declare function local:substring-before-last( $arg as xs:string? ,$delim as xs:string )  as xs:string {
   ...
   then replace($arg,  concat('^(.*)', local:escape-for-regex($delim),'.*'),'$1')
   else ''
 } ;
...
declare %private function unzip:store-entry($path as xs:string, $data-type as xs:string, $data as item()?, $param as item()*) as element() {
    ...
    return
        if ($data-type = 'folder') then
            ...
            let $resource-collection := concat($unzip-base-collection, '/', xmldb:encode(local:substring-before-last($path, '/')))
            let $resource-filename := if (contains($path, '/')) then local:substring-after-last($path, '/') else $path
...
declare 
%rest:GET
%rest:path("/unzip/list")
%rest:query-param("resource", "{$resource}","")
function unzip:list($resource as xs:string*) as element(entries) {
...
%rest:GET
%rest:path("/unzip")
%rest:query-param("resource", "{$zip-file}","")
function unzip:unzip($zip-file as xs:string*) as element(entries) {
    let $zip-file-collection := local:substring-before-last($zip-file, '/')
...
%rest:path("/unzip")
%rest:query-param("resource", "{$zip-file}","")
%rest:query-param("target-collection ", "{$target-collection}","")
function unzip:unzip($zip-file as xs:string*, $target-collection as xs:string*) as element(entries) {
